...
Indexing also feeds two dashboard features that otherwise stay empty: the **Task given** panel on the [Sub-agents](/docs/subagents/) tab, which is the first user message sent to a sub-agent, and the inline error text under failing tool calls in the session [Timeline](/docs/session/).
