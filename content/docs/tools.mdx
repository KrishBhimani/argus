...
Calls to the `Task` or `Agent` tool grouped by the sub-agent type that was requested. To see what a particular sub-agent did, open its parent session's [Sub-agents](/docs/subagents/) tab.

## Tools feed the alert detector

The one shipped detector watches per-tool error rates and raises an alert when a tool's rate doubles against its baseline. See [Alerts](/docs/alerts/).
