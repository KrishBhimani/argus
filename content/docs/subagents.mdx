...
**Task given** is the first user message the sub-agent received, which is the instruction the parent wrote for it. This needs [transcript indexing](/docs/search/); without it the panel reads "Not available — enable transcript indexing in Settings to keep sub-agent prompts."
...
The [Tools](/docs/tools/) page has a **Sub-agent invocations** panel that counts delegations by sub-agent type across every session in the window.
